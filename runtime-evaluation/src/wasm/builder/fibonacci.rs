...
pub struct FibonacciReactorFunctionBuilder;
impl WasmFunctionBuilder for FibonacciReactorFunctionBuilder {
    type InputType = i32;
    ...
    fn build_function(&self, workload: &Workload<i32>) -> Box<dyn Fn(&Linker<WasmExecutionState>, &mut Store<WasmExecutionState>) -> Result<()>> {
        ...
        let function = move |linker: &Linker<WasmExecutionState>, mut store: &mut Store<WasmExecutionState>| {
            let wasm_fun = linker.get(&mut store, "", "fibonacci").unwrap().into_func().unwrap().typed::<i32, i64, _>(&store)?;
...
pub struct FibonacciCommanderFunctionBuilder;
impl WasmFunctionBuilder for FibonacciCommanderFunctionBuilder {
    type InputType = i32;
    ...
    fn get_arguments(&self, workload: &Workload<i32>) -> Vec<String> {
        vec![String::from(workload.language.to_string()+ ".wasm"), String::from("fibonacci.rb"), workload.size.value.to_string()]
    }
