...
build() {
    ...
    docker run --rm -v "$PWD":/build build_"$language" \
      /opt/wasi-sdk/bin/clang "$workload_name".c -o "$workload".wasm $opt_parameter -Wl,--allow-undefined
...
print_versions() {
    docker run --rm build_"$language" printenv WASI_VERSION_FULL
    docker run --rm build_"$language" /opt/wasi-sdk/bin/clang --version
}
